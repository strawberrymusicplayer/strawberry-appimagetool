...
if [ ! -z $(find "${HERE}" -name "libgstcoreelements.so" -type f) ] ; then
  ...
  export GST_PLUGIN_SCANNER=$(find "${HERE}" -type f \( -name "gst-plugin-scanner.sh" -o -name "gst-plugin-scanner-x86_64.sh" \) | head -n 1)
...
if [ -e "$LD_LINUX" ] ; then
  ...
  export QT_PLUGIN_PATH="${HERE}"/usr/lib64/qt5/plugins/:"${HERE}"/usr/lib64/qt6/plugins/:"${QT_PLUGIN_PATH}"
...
func AppDirDeploy(path string) {
  ...
  */
  ...
  for _, lib := range allELFs {
    ...
    if strings.Contains(lib, "libQt5Core.so.5") || strings.Contains(lib, "libQt6Core.so.6") {
...
func patchQtPrfxpath(appdir helpers.AppDir, lib string, libraryLocationsInAppDir []string, ldLinux string) {
  ...
  defer f.Close()
  if err != nil {
    helpers.PrintError("Could not open libQtCore for reading", err)
    os.Exit(1)
  }
  f.Seek(0, 0)
  ...
  */
  ...
  defer f.Close()
  if err != nil {
    helpers.PrintError("Could not open libQtCore for writing", err)
    os.Exit(1)
  }
  // Now that we know where in the file the information is, go write it
  f.Seek(offset, 0)
  if quirksModePatchQtPrfxPath == false {
    log.Println("Patching qt_prfxpath in libQtCore to " + relPathToQt)
    _, err = f.Write([]byte(relPathToQt + "\x00"))
  } else {
    log.Println("Patching qt_prfxpath in libQtCore to ..")
  ...
  for _, lib := range allELFs {
    if strings.HasPrefix(filepath.Base(lib), "libgstreamer-1.0") {
      ...
      if err != nil {
        ...
        for _, gstplugin := range gstPlugins {
          determineELFsInDirTree(appdir, "/usr/lib64/gstreamer-1.0/" + gstplugin)
        }
...
func handleQt(appdir helpers.AppDir, qtVersion int) {
  ...
  var qtcorelib="libQt" + strconv.Itoa(qtVersion) + "Core.so." + strconv.Itoa(qtVersion)
  var qtguilib="libQt" + strconv.Itoa(qtVersion) + "Gui.so." + strconv.Itoa(qtVersion)
  //var qtnetworklib="libQt" + strconv.Itoa(qtVersion) + "Network.so." + strconv.Itoa(qtVersion)
  var qtsqllib="libQt" + strconv.Itoa(qtVersion) + "Sql.so." + strconv.Itoa(qtVersion)

    // Actually the qtcorelib contains (always?) qt_prfxpath=... which tells us the location in which 'plugins/' is located

    library, err := findLibrary(qtcorelib)
    if err != nil {
      helpers.PrintError("Could not find " + qtcorelib, err)
      os.Exit(1)
    }
    ...
    defer f.Close()
    if err != nil {
      helpers.PrintError("Could not open " + qtcorelib, err)
      os.Exit(1)
    }

    qtPrfxpath := getQtPrfxpath(qtVersion)
    ...
    for _, lib := range allELFs {
      if strings.HasSuffix(lib, qtguilib) == true {
    ...
    //for _, lib := range allELFs {
      //if strings.HasSuffix(lib, qtnetworklib) == true {
    ...
    for _, lib := range allELFs {
      if strings.HasSuffix(lib, qtsqllib) == true {
        determineELFsInDirTree(appdir, qtPrfxpath+"/plugins/sqldrivers/libqsqlite.so")
        break
      }
    }
...
func getQtPrfxpath(qtVersion int) string {

  qt_prfxpath := ""
  if qtVersion == 5 {
    qt_prfxpath = "/usr/lib64/qt5"
  } else {
    qt_prfxpath = "/usr/lib64/qt6"
  }
